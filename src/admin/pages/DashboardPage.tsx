...
import { Box, Grid, Skeleton, Typography, keyframes, styled, Paper } from "@mui/material";
...
const SectionTitle = styled(Typography)<{ $color: string }>(({ $color }) => ({
  ...
  padding: "12px 16px",
  borderRadius: "10px",
  background: `linear-gradient(135deg, rgba(${hexToRgb($color)}, 0.08) 0%, rgba(${hexToRgb($color)}, 0.02) 100%)`,
  border: `1px solid rgba(${hexToRgb($color)}, 0.15)`,
  "&::after": {
    content: '""',
    flex: 1,
    height: "1px",
    background: `linear-gradient(90deg, rgba(${hexToRgb($color)}, 0.4), rgba(${hexToRgb($color)}, 0.1), transparent)`,
  },
}));
